...
def wl_rw(addr, cmd, var, val):
    ...
    if EMULATION:
        ...
        if config.dobby['OS'] == 'WIN':
            ...
            res = subprocess.run(["wl_send_cmd", str(hex(addr)), cmd, var, str(val), d, l], stdout=subprocess.PIPE,
                                 encoding='utf-8')
...
def pump_onoff(pump_name, onoff):
    ...
    bit_str = '000100000000' + config.pump[config.pump_fieldnames[4]] + config.pump[config.pump_fieldnames[3]] + \
              config.pump[config.pump_fieldnames[2]] + config.pump[config.pump_fieldnames[1]]
